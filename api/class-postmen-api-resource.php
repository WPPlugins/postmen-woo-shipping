...
class Postmen_API_Resource {
	 ...
	 * @since 2.1
	 *
	 * @param Postmen_API_Server $server
	 *
	 */
	public function __construct( Postmen_API_Server $server ) {

		$this->server = $server;

		// automatically register routes for sub-classes
		add_filter( 'postmen_api_endpoints', [ $this, 'register_routes' ] );
		...
		foreach ( [ 'order', 'coupon', 'customer', 'product', 'report' ] as $resource ) {
			...
			add_filter( "postmen_api_{$resource}_response", [ $this, 'maybe_add_meta' ], 15, 2 );
			add_filter( "postmen_api_{$resource}_response", [ $this, 'filter_response_fields' ], 20, 3 );
		}
	}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) ) {
			return new WP_Error( "postmen_api_invalid_{$resource_name}_id",
				sprintf( __( 'Invalid %s ID', 'postmen' ), $type ), [ 'status' => 404 ] );
		}
		...
		if ( 'customer' !== $type ) {
			...
			// validate post type
			if ( $type !== $post_type ) {
				return new WP_Error( "postmen_api_invalid_{$resource_name}",
					sprintf( __( 'Invalid %s', 'postmen' ), $resource_name ), [ 'status' => 404 ] );
			}
			...
			switch ( $context ) {
				...
				case 'read':
					if ( ! $this->is_readable( $post ) ) {
						return new WP_Error( "postmen_api_user_cannot_read_{$resource_name}",
							sprintf( __( 'You do not have permission to read this %s', 'postmen' ), $resource_name ),
							[ 'status' => 401 ] );
					}
					break;
				...
				case 'edit':
					if ( ! $this->is_editable( $post ) ) {
						return new WP_Error( "postmen_api_user_cannot_edit_{$resource_name}",
							sprintf( __( 'You do not have permission to edit this %s', 'postmen' ), $resource_name ),
							[ 'status' => 401 ] );
					}
					break;
				...
				case 'delete':
					if ( ! $this->is_deletable( $post ) ) {
						return new WP_Error( "postmen_api_user_cannot_delete_{$resource_name}",
							sprintf( __( 'You do not have permission to delete this %s', 'postmen' ), $resource_name ),
							[ 'status' => 401 ] );
					}
					break;
			}
	...
	protected function merge_query_args( $base_args, $request_args ) {
		...
		if ( ! empty( $request_args['created_at_min'] ) || ! empty( $request_args['created_at_max'] ) || ! empty( $request_args['updated_at_min'] ) || ! empty( $request_args['updated_at_max'] ) ) {
			...
			if ( ! empty( $request_args['updated_at_min'] ) ) {
				$args['date_query'][] = [ 'column'    => 'post_modified_gmt',
	...
	protected function delete( $id, $type, $force = false ) {
		...
		if ( 'customer' === $type ) {
			...
			if ( $result ) {
				return [ 'message' => __( 'Permanently deleted customer', 'postmen' ) ];
			} else {
				return new WP_Error( 'postmen_api_cannot_delete_customer',
					__( 'The customer cannot be deleted', 'postmen' ), [ 'status' => 500 ] );
			}
			...
			if ( ! $result ) {
				return new WP_Error( "postmen_api_cannot_delete_{$resource_name}",
					sprintf( __( 'This %s cannot be deleted', 'postmen' ), $resource_name ), [ 'status' => 500 ] );
			}

			if ( $force ) {
				return [ 'message' => sprintf( __( 'Permanently deleted %s', 'postmen' ), $resource_name ) ];

			} else {

				$this->server->send_status( '202' );

				return [ 'message' => sprintf( __( 'Deleted %s', 'postmen' ), $resource_name ) ];
			}
		}
	}
