...
class Postmen_API_Server {
	...
	public function __construct( $path ) {
		...
		if ( $this->is_json_request() ) {
			$handler_class = 'Postmen_API_JSON_Handler';
		} elseif ( $this->is_xml_request() ) {
			$handler_class = 'WC_API_XML_Handler';
		} else {
			$handler_class = apply_filters( 'postmen_api_default_response_handler', 'Postmen_API_JSON_Handler',
				$this->path, $this );
		}
	...
	public function check_authentication() {
		...
		$user = apply_filters( 'postmen_api_check_authentication', null, $this );
		...
		elseif ( ! is_wp_error( $user ) ) {
			$user = new WP_Error( 'postmen_api_authentication_error', __( 'Invalid authentication method', 'postmen' ),
				[ 'code' => 500 ] );
		}
	...
	public function serve_request() {
		...
		do_action( 'postmen_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'postmen_api_enabled', true,
				$this ) || ( 'no' === get_option( 'postmen_api_enabled' ) ) ) {

			$this->send_status( 404 );

			echo $this->handler->generate_response( [
				'errors' => [
					'code'    => 'postmen_api_disabled',
		...
		$served = apply_filters( 'postmen_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'postmen_api_endpoints', $endpoints );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			case 'POST':
				...
				break;
			default:
				return new WP_Error( 'postmen_api_unsupported_method', __( 'Unsupported request method', 'postmen' ),
					[ 'status' => 400 ] );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) ) {
					return new WP_Error( 'postmen_api_invalid_handler',
						__( 'The handler for the route is invalid', 'postmen' ), [ 'status' => 500 ] );
				}
				...
				$args = apply_filters( 'postmen_api_dispatch_args', $args, $callback );
		...
		return new WP_Error( 'postmen_api_no_route',
			__( 'No route was found matching the URL and request method', 'postmen' ), [ 'status' => 404 ] );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			if ( isset( $provided[ $param->getName() ] ) ) {
				...
				//$ordered_parameters[] = is_array($provided[$param->getName()]) ? $provided[$param->getName()] : urldecode($provided[$param->getName()]);
				$ordered_parameters[] = is_array( $provided[ $param->getName() ] ) ? $provided[ $param->getName() ] : $provided[ $param->getName() ];
				...
				return new WP_Error(
					'postmen_api_missing_callback_param',
					sprintf( __( 'Missing parameter %s', 'postmen' ), $param->getName() ),
					[ 'status' => 400 ]
				);
			}
		}
			...
			foreach ( self::$method_map as $name => $bitmask ) {
				foreach ( $callbacks as $callback ) {
					...
					if ( strpos( $route, '<' ) === false ) {
						$data['meta'] = [
							'self' => get_postmen_api_url( $route ),
						];
					}
				}
			}
			$available['store']['routes'][ $route ] = apply_filters( 'postmen_api_endpoints_description', $data );
		}

		return apply_filters( 'postmen_api_index', [] );
	}
	...
	public function add_pagination_headers( $query ) {
		...
		do_action( 'postmen_api_pagination_headers', $this, $query );
	}
