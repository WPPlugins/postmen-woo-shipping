...
class PostmenWoocommercePlugin_OptionsManager {
	...
	public function settingsPage() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'postmen-woocommerce-plugin' ) );
		}
		...
		?>
        <div class="wrap">
            <h2><?php _e( 'System Settings', 'postmen-woocommerce-plugin' ); ?></h2>
            <table cellspacing="1" cellpadding="2">
                <tbody>
                <tr>
                    <td>d<?php _e( 'System', 'postmen-woocommerce-plugin' ); ?></td>
                    <td><?php echo php_uname(); ?></td>
                </tr>
                <tr>
                    <td><?php _e( 'PHP Version', 'postmen-woocommerce-plugin' ); ?></td>
                ...
                <tr>
                    <td><?php _e( 'MySQL Version', 'postmen-woocommerce-plugin' ); ?></td>
            ...
            <h2><?php echo $this->getPluginDisplayName();
				echo ' ';
				_e( 'Settings', 'postmen-woocommerce-plugin' ); ?></h2>
            ...
            <form method="post" action="">
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e( 'Save Changes', 'postmen-woocommerce-plugin' ) ?>"/>
                </p>
            </form>
            <p>get orders</p>
			<?php
			//	$sth = postmen_connector_get_orders();
			//	$order_id = $sth['orders'][0]['id'];
			//	$order_status = $sth['orders'][0]['status'];
			//	echo json_encode($sth);
			?>
            <p>update order status (refresh the page, first order status is swapping between processing and pending)</p>
			<?php
			...
			//	}
			//	postmen_connector_update_order_status($order_id, $order_status_new);
			?>
            <p>create order note (private note)</p>
			<?php
			//	postmen_connector_add_order_note($order_id, false, 'new private note');
			?>

            <p>create order note (customer note)</p>
			<?php
			//	postmen_connector_add_order_note($order_id, true, 'new public note');
			?>

            <p>get order notes</p>
			<?php
			//	echo json_encode(postmen_connector_get_order_notes($order_id));
			?>
            <p>is AfterShip Woocommerce tracking enabled?</p>
			<?php
			//	echo postmen_connector_enable_tracking($order_id, 'ups', '1ZE8316Y0341233717');
			?>
            <p>weight unit</p>
			<?php
			/* also works for woocommerce_dimension_unit */
			//	echo get_option( 'woocommerce_weight_unit' );
			?>
            <p>product number 10 info</p>
			<?php
			//	$product = new WC_Product(10);
			//	echo 'weight is ' . $product->get_weight() . '</br>';
			//	echo 'description is ' . get_post(10)->post_content . '</br>';
			?>
        </div>
		<?php
	...
	protected function getOptionValueI18nString( $optionValue ) {
		switch ( $optionValue ) {
			case 'true':
				return __( 'true', 'postmen-woocommerce-plugin' );
			case 'false':
				return __( 'false', 'postmen-woocommerce-plugin' );

			case 'Administrator':
				return __( 'Administrator', 'postmen-woocommerce-plugin' );
			case 'Editor':
				return __( 'Editor', 'postmen-woocommerce-plugin' );
			case 'Author':
				return __( 'Author', 'postmen-woocommerce-plugin' );
			case 'Contributor':
				return __( 'Contributor', 'postmen-woocommerce-plugin' );
			case 'Subscriber':
				return __( 'Subscriber', 'postmen-woocommerce-plugin' );
			case 'Anyone':
				return __( 'Anyone', 'postmen-woocommerce-plugin' );
		}
